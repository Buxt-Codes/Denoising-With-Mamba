...
class MambaBlockFiLm(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            d_context (int): The dimension of the input context.
...
class MambaBlockCrossAttn(nn.Module):
    '''
    Class representing the MambaBlock as introduced in Gu & Dao (2023).
    '''
    ...
    def __init__(
        ...
        # Projection matrices for endowing the context stage with
        # context-dependent capability (i.e. input dependence)
        self.context_B = nn.MultiheadAttention(d_model, d_heads, dropout=dropout, batch_first=True)
        self.context_C = nn.MultiheadAttention(d_model, d_heads, dropout=dropout, batch_first=True)
    ...
    def ssm(self, seq : Tensor, context : Tensor, prev_hid : Tensor | None) -> Tuple[Tensor, Tensor]:
        ...
        attn_B, _ = self.context_B(seq, context, context)  
        attn_C, _ = self.context_C(seq, context, context)

        B = self.s_B(seq) * attn_B
        C = self.s_C(seq) * attn_C
        Δ = softplus(D + self.s_D(seq)) # shape: (batch_size, seq_len, d_model)
